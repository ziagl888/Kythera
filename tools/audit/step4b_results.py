...
sec("Dedup analysis: who generates the duplicates?")
grp = ai.groupby(["symbol", "model", "direction", "open_time"]).size()
dups = grp[grp > 1]
print(f"Total groups: {len(grp)}, duplicate groups: {len(dups)}, excess rows: {int((dups-1).sum())} of {len(ai)}")
...
print(f"\nRows with sentinel open_time 2026-02-24 12:43:59.65: {len(sent)} (migration/default timestamp?)")

sec("AI results DEDUPLICATED (first close row per group; active era, win/loss)")
...
print(f"\nTOTAL dedup: n={len(cur)}, WR={100*(cur.cls=='win').mean():.1f}%, avg {cur.pnl_pct.mean():.2f}%, "
      f"sum {cur.pnl_pct.sum():.0f}% (net {cur.pnl_pct.sum()-FEE_RT*len(cur):.0f}%)")

leg = dedup[(dedup.cls == "legacy") & (dedup.entry > 0) & (dedup.close_price > 0)]
legwin = leg.status.str.upper().str.contains("TARGET")
print(f"\nLEGACY dedup: n={len(leg)}, TARGET-HIT rate {100*legwin.mean():.1f}%, avg {leg.pnl_pct.mean():.2f}%, sum {leg.pnl_pct.sum():.0f}%")
print("LEGACY time range:", str(leg.close_time.min())[:10], "to", str(leg.close_time.max())[:10])
...
dupc = grpc[grpc > 1]
print(f"Duplicate groups: {len(dupc)}, excess rows: {int((dupc-1).sum())}")
...
sec("Classic DEDUP: results per strategy (win = >=TP1)")
...
print(f"\nTOTAL Classic dedup: n={len(cc)}, WR={100*(cc.cls=='win').mean():.1f}%, avg {cc.pnl_pct.mean():.2f}%, sum {cc.pnl_pct.sum():.0f}%")

sec("Classic: status-vs-PnL consistency")
for lab, sub in (("status=win but pnl<-0.5%", cc[(cc.cls == "win") & (cc.pnl_pct < -0.5)]),
                 ("status=loss but pnl>+0.5%", cc[(cc.cls == "loss") & (cc.pnl_pct > 0.5)])):
...
sec("Classic: censoring shares (censored incl. FORCE_CLOSED)")
