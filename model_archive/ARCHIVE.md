...
> Generated by `tools/bot_variants/archive.py` (T-2026-KYT-9050-039). **Do not edit by hand** — regenerate with `python -m tools.bot_variants.archive --write`.
>
> Reference-based: the artifact bytes are git-tracked in root/staging; per generation, `manifest.json` holds md5 + `source_commit` ⇒ retrieval via `git show <source_commit>:<path>`. `--copy-binaries` produces a self-contained export.
