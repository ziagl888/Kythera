...
# FIX: load token from environment instead of hardcoded.
...
async def global_command_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    This function catches ALL text messages, whether in a group or a channel!
    """
    ...
    # In channels there is often no "user" writing, but the channel itself. Fallback:
    user = update.effective_user
    username = user.username if user else (user.full_name if user else "Channel_Admin")

    # 1. Check permission system
    ...
    # 3. Forward to the correct handler
    try:
        if command_name == "!open":
            await open_command_callback(update, context)
        elif command_name == "!ping":
            await msg.reply_text("🏓 Pong! The new system is running smoothly.")
        else:
        ...
        await msg.reply_text("❌ There was an internal error during execution.")
