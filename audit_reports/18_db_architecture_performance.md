...
- [x] **✅(2026-07-04: Code-Hebel umgesetzt — WHERE IS DISTINCT FROM in beiden Upsert-Pfaden von 1_data_ingestion.py; wal_compression=on + R1/TimescaleDB stehen aus) D3 (MEDIUM): Write-Amplification des 3s-Upsert-Loops gemessen:** jede 5m-Kerze wird **~15× überschrieben** (pro 5m-Tabelle 52.945 Updates auf 3.503 Inserts im Fenster, × 698 Symbole ≈ 2,8 Mio Updates/Tag nur für 5m). Zusammen mit dem 12h-Catch-up (7-Tage-Rewrite) erklärt das die 8,5 GB WAL/Tag. **Fix kurzfristig:** Upsert nur bei geändertem Close/Volume (`WHERE`-Klausel im UPDATE-Teil), `wal_compression=on` (eine Zeile, ~50% WAL-Ersparnis); **richtig gelöst** wird es durch R1 (nur geschlossene Kerzen schreiben) bzw. Abschnitt B.
