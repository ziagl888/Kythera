...
# How many TPs ROM1 publishes AND persists (T-2026-KYT-9050-099). Bound to the
# shared constant rather than a literal 3: it is the same number the thinning has
# to reach (`thin_targets(..., keep=)`) and the same number the ai_signals insert
# stores — a local literal is how those three drifted apart in the first place.
ROM1_PUBLISHED_TARGETS = N_PUBLISHED_TARGETS


def rom1_published_targets(targets: list) -> list:
    """The TPs ROM1 actually publishes — and therefore the ones it persists.

    ONE source for the Cornix loop and the ``ai_signals`` insert
    (T-2026-KYT-9050-099). Before, the message sliced ``[:ROM1_PUBLISHED_TARGETS]``
    and the insert stored the full ``t_cands[:20]`` pool, which is the persist ≠
    publish gap T-2026-KYT-9050-012 measured (position model off by factor 1.41,
    139 of 7,774 trades scored with ``targets_hit > 3`` for TPs Cornix never had).
    Keeping the slice in one function is what makes ``len(ai_signals.targets) ==
    number of published TPs`` structural instead of a coincidence of two call
    sites — the P2.31 invariant the rest of the fleet already holds.

    Note this runs AFTER ``ensure_min_tp_distance``: on a sparse pool the thinner
    returns fewer than ``keep`` levels and the 5 % backstop appends one, so the
    published ladder can be shorter than ``ROM1_PUBLISHED_TARGETS`` — that is the
    honest count, and persist follows publish either way.
    """
    return list(targets)[:ROM1_PUBLISHED_TARGETS]
...
def compute_rom1_trade_params(conn, coin: str, direction: str, price=None, df=None) -> dict | None:
    ...
    # Up to 20 candidates (like the AI bots), thinned to the published count and
    # then capped by ensure_min_tp_distance.
    #
    # T-2026-KYT-9050-099: `t_cands` is filtered against the ENTRY only
    # (`x > entry1 * 1.01` above) — the first hop is guaranteed 1 %, every hop
    # after it is whatever the raw S/R list happens to give. Measured on 8,144
    # ROM1 signals since the P2.31 fix: median gap TP1→TP2 1.00 %, TP2→TP3 0.94 %,
    # 50/52 % of those gaps under 1 %, and in 20.7 % of signals the WHOLE
    # TP1..TP3 ladder spans less than 1 % — three Cornix tranches inside one tick.
    # `thin_targets` (T-2026-KYT-9050-098) reaches deeper into the pool instead;
    # TP1 is never moved (the first candidate is always kept), only TP2/TP3.
    targets = ensure_min_tp_distance(
        thin_targets(t_cands[:20], entry1, is_long, keep=ROM1_PUBLISHED_TARGETS),
        entry1,
        is_long,
        min_pct=ROM1_TP_MIN_DISTANCE_PCT,
    )
...
def insert_rom1_signal(conn, coin: str, direction: str, params: dict, commit: bool = True) -> None:
    ...
    T-2026-KYT-9050-099: stores exactly the PUBLISHED ladder
    (``rom1_published_targets``), not the whole ``t_cands[:20]`` pool. This closes
    the last persist ≠ publish gap named in P2.31 / T-2026-KYT-9050-012 and has
    three consequences the operator gated:

      * Monitor 8 scores ``len(stored)``, so ALL TARGETS HIT now fires at TP3
        instead of at a rung Cornix never received. Measured over 9,123 closed
        ROM1 trades since 2026-07-11: 6.4 % reach TP3 (today only 0.4 % close as
        ALL TARGETS HIT), and 1.8 % were scored ``targets_hit > 3`` — credit for
        TPs that were never in the message. That last class disappears at the
        source.
      * The monitor's SL trailing and ``_compute_trailed_sl`` (regime auto-close,
        which posts a REAL Cornix ``SL`` command) index into this list. They now
        trail to published levels only; before, a ``targets_hit > 3`` row trailed
        to a phantom rung.
      * ``core.realized_pnl.traded_targets`` keeps working for BOTH eras: it takes
        ``targets[:3]``, which is identity on the new rows and still the posted
        three on every historical row. That equivalence is exactly what a
        thin-only-the-message variant would have broken (see the task doc), and
        it is pinned in ``backtest/test_traded_targets.py``.
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            (
                ...
                params["entry2"],
                params["sl"],
                json.dumps(rom1_published_targets(params["targets"])),
                utc_now_naive(),
            ),
        )
    if commit:
...
def _gate_and_forward_row(
    ...
    logger.info(
        ...
        f"(SL {rom1_params['sl']:.6f}, {len(rom1_published_targets(rom1_params['targets']))} TPs) "
