...
MARKETS = {
    ...
    "FOREX": {
        ...
        "pairs": [
            ...
            'USDCAD=X', 'USDCHF=X', 'NZDUSD=X', 'EURGBP=X', 'GBPJPY=X', 'GC=F', 'SI=F'
        ]
    }
}
...
def fetch_db_data(conn, symbol, tf):
    try:
        query = f'SELECT open_time, open, high, low, close, volume FROM "{symbol}_{tf}" ORDER BY open_time DESC LIMIT 300'
...
def fetch_yfinance_data(ticker, tf):
    try:
        ...
        if tf == '15m': yf_interval = '15m'; period = '30d'
        elif tf == '30m': yf_interval = '30m'; period = '30d'
        elif tf == '1h': yf_interval = '1h'; period = '60d'
        elif tf == '2h': yf_interval, resample_tf = '1h', '2h'
        elif tf == '4h': yf_interval, resample_tf = '1h', '4h'
        elif tf == '1d': yf_interval, period = '1d', '200d'
        elif tf == '1w': yf_interval, period = '1wk', '400d'
        ...
        df = df.reset_index()
        col_map = {'Datetime': 'open_time', 'Date': 'open_time', 'Open': 'open', 'High': 'high', 'Low': 'low', 'Close': 'close', 'Volume': 'volume'}
...
def find_unmitigated_fvgs(df, direction="BULLISH"):
    ...
    for fvg in fvgs:
        ...
        for j in range(fvg['index'] + 1, len(df)): # 💥 FIX: Bis zum tatsächlichen Ende iterieren!
            if direction == "BULLISH" and df['low'].iloc[j] <= fvg['top']: is_mitigated = True; break
            if direction == "BEARISH" and df['high'].iloc[j] >= fvg['bottom']: is_mitigated = True; break
        if not is_mitigated: unmitigated.append(fvg)
    return unmitigated
...
def generate_smc_chart(df, symbol, tf, setup_type, level_data, is_bullish, supports, resistances):
    try:
        ...
        mpf.plot(
            ...
            title=f"\n{title}", figsize=(12, 8), tight_layout=True, savefig=filename, returnfig=False
        )
        return filename
    except Exception as e:
...
def send_signal(conn, channel, symbol, direction, price, targets, sl, setup_type, chart_path, tf, details):
    ...
    lines = [f"📈 SMC Signal for {symbol} 📈", f"🚨 Direction: {direction}", f"🚨 Leverage: 20x-10x",
             f"🏦 CMP Entry: $ {price:.5f}", f"🏦 Entry 2: $ {price * (0.98 if direction == 'LONG' else 1.02):.5f}"]
    ...
    with conn.cursor() as cur:
        if chart_path:
            cur.execute("INSERT INTO telegram_outbox (channel_id, message, image_path) VALUES (%s, %s, %s)", (channel, html, chart_path))
        else:
...
def run_smc_analysis():
    ...
    for market_name, config in MARKETS.items():
        ...
        for symbol in pairs:
            ...
            if symbol == 'GC=F': display_name = 'GOLD'
            elif symbol == 'SI=F': display_name = 'SILVER'
            elif symbol == 'JPY=X': display_name = 'USDJPY'
                    ...
                    if recent_res and recent_sup:
                        ...
                        if price > last_res_val and df['open'].iloc[curr_idx] <= last_res_val:
                            ...
                            if not check_cooldown(conn, cd_key, display_name, 'LONG', 12):
                                ...
                                tgts = sorted([v for i, v in resistances if v > price])[:5] or [price * 1.01, price * 1.02]
                                sl = last_sup_val * 0.998
                                c_path = generate_smc_chart(df, display_name, tf, "STRUCTURE", {'price': last_res_val, 'index': last_res_idx}, True, supports, resistances)
                                send_signal(conn, channel_id, display_name, "LONG", price, tgts, sl, "STRUCTURE", c_path, tf, f"Break of Swing High @ {last_res_val:.5f}")
                        ...
                        # BEARISH SHIFT
                            ...
                            if not check_cooldown(conn, cd_key, display_name, 'SHORT', 12):
                                ...
                                tgts = sorted([v for i, v in supports if v < price], reverse=True)[:5] or [price * 0.99, price * 0.98]
                                sl = last_res_val * 1.002
                                c_path = generate_smc_chart(df, display_name, tf, "STRUCTURE", {'price': last_sup_val, 'index': last_sup_idx}, False, supports, resistances)
                                send_signal(conn, channel_id, display_name, "SHORT", price, tgts, sl, "STRUCTURE", c_path, tf, f"Break of Swing Low @ {last_sup_val:.5f}")
                    ...
                    for fvg in bull_fvgs[-2:]:
                        if curr_candle['low'] <= fvg['top'] and price > (fvg['bottom'] * 0.999):
                            ...
                            if not check_cooldown(conn, cd_key, display_name, 'LONG', 12):
                                tgts = sorted([v for i, v in resistances if v > price])[:5] or [price * 1.01, price * 1.02]
                                sl = fvg['bottom'] * 0.998
                                c_path = generate_smc_chart(df, display_name, tf, "FVG", fvg, True, supports, resistances)
                                send_signal(conn, channel_id, display_name, "LONG", price, tgts, sl, "FVG", c_path, tf, f"Tapped into BISI @ {fvg['top']:.5f}")
                                update_cooldown(conn, cd_key, display_name, 'LONG')
                    ...
                    for fvg in bear_fvgs[-2:]:
                        if curr_candle['high'] >= fvg['bottom'] and price < (fvg['top'] * 1.001):
                            ...
                            if not check_cooldown(conn, cd_key, display_name, 'SHORT', 12):
                                tgts = sorted([v for i, v in supports if v < price], reverse=True)[:5] or [price * 0.99, price * 0.98]
                                sl = fvg['top'] * 1.002
                                c_path = generate_smc_chart(df, display_name, tf, "FVG", fvg, False, supports, resistances)
                                send_signal(conn, channel_id, display_name, "SHORT", price, tgts, sl, "FVG", c_path, tf, f"Tapped into SIBI @ {fvg['bottom']:.5f}")
                                update_cooldown(conn, cd_key, display_name, 'SHORT')
