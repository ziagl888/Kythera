...
SYMBOL = 'BTCUSDT'
TIMEFRAME = '1h'
OUTPUT_FILE = "btc_rsi_optimization_results.txt"
...
# ==========================================
def run_simulation(df, sl_pct, min_rr_ratio, rsi_limit):
...
def main():
    ...
    combinations = list(itertools.product(SL_PCTS, MIN_RR_RATIOS, RSI_LIMITS))
    ...
    for r in results[:10]:
        print(
            f"{r['sl'] * 100:>5.2f}%  | {r['rr']:<6.2f} | <{r['rsi']:<4} | {r['trades']:<6} | {r['win_rate']:>6.2f} % | {r['max_dd']:>5.2f}% | ${r['pnl']:+,.2f}")
