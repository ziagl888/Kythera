...
from core.hosted_fleet import ALL_HOSTED_SCRIPTS  # noqa: E402
...
def test_mapped_scripts_exist_in_fleet():
    ...
    # process, or (since T-2026-KYT-9050-133/135) hosted inside one of the
    # shared runners. Anything else is a tag pointing at a bot nobody executes.
    runnable = {e["script"] for e in FLEET} | set(ALL_HOSTED_SCRIPTS)
...
def test_active_scripts_excludes_parked():
    ...
    # One fleet entry parked, plus every bot the two runners host — those have
    # no fleet entry of their own since T-2026-KYT-9050-133/135 but do run.
    assert len(active) == len(FLEET) - 1 + len(ALL_HOSTED_SCRIPTS)
