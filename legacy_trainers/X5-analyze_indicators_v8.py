...
async def train_all_horizons_final():
    ...
    # PUMP + DUMP Horizonte (jeweils Threshold in % Preisbewegung)
    horizons = [
        ("8h_pump", 8, 5.0),     # +5% in 8h → Pump
        ("8h_dump", 8, -5.0),    # -5% in 8h → Dump
        ("24h_pump", 24, 10.0),     # +5% in 8h → Pump
        ("24h_dump", 24, -10.0),    # -5% in 8h → Dump
        ("72h_pump", 72, 15.0),
        ("72h_dump", 72, -15.0),
        ("168h_pump", 168, 25.0),
        ("168h_dump", 168, -25.0),
    ...
    for name, hours, threshold_pct in horizons:
        ...
        logger.info(f"BESTE Precision für {name}: {best_precision:.3f} bei Recall {best_recall:.3f} @ Threshold {best_threshold:.3f}")
